...
DubinsCurvesHandler::DubinsCurvesHandler(double k_max, double discritizer_size){
    this->k_max = k_max;
...
DubinsCurve DubinsCurvesHandler::calculateDubinsCurve(double x0, double y0, double th0, double s1, double s2, double s3, double k0, double k1, double k2){
    ...
    dubins_curve.discretized_curve = discretizeDubinsCurve(x0, y0, th0, dubins_curve.arcs);
...
std::vector<DubinsLine> DubinsCurvesHandler::discretizeDubinsCurve(double starting_x, double starting_y, double starting_theta, DubinsArc arcs[3])
{
    int npts = arcs[0].L / discritizer_size;
    std::vector<DubinsLine> discretized_curve;
    //int npts = 100;

    for(int i = 0; i < npts; i++)
    {
        double s = arcs[0].L / npts * i;

        DubinsLine discretized_line = calculateDubinsLine(s, starting_x, starting_y, starting_theta, arcs[0].k);
        discretized_curve.push_back(discretized_line);
    }

    for(int a = 1; a < 3; a++)
    {
        npts = arcs[a].L / discritizer_size;
        for(int i = 0; i < npts; i++)
        {
            double s = arcs[a].L / npts * i;

            DubinsLine discretized_line = calculateDubinsLine(s, arcs[a - 1].xf, arcs[a - 1].yf, arcs[a - 1].thf, arcs[a].k);
            discretized_curve.push_back(discretized_line);
        }
    }

    // Adding remainder
    DubinsLine discretized_line = calculateDubinsLine(arcs[2].L, arcs[1].xf, arcs[1].yf, arcs[1].thf, arcs[2].k);
    discretized_curve.push_back(discretized_line);
