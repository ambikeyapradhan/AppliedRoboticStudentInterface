...
//     /*PRM sampling motion planning debugging*/
...
void UT_dubins_collision_test(struct arc_extract three_seg[3],
  ...
  for (size_t i = 0; i<obstacle_list.size(); i++){
        ...
        for (size_t j = 0; j<output.size()-1; j++){ //4corners of each box
           //std::cout <<"BOX start_point.x= " << output[j].x << "start_point.y= " << output[j].y<< "end_point.y= " << output[j+1].x << "end_point.y= " << output[j+1].y << std::endl; 
        }
  ...
  for (size_t i = 0; i<obstacle_list.size(); i++){//no of boxes 
        for (size_t j = 0; j<obstacle_list[i].size()-1; j++){ //4corners of each box
            ...
            //std::cout <<"Polygon start_point.x= " << obstacle_list[i][j].x << "start_point.y= " << obstacle_list[i][j].y<< "end_point.y= " << obstacle_list[i][j+1].x << "end_point.y= " << obstacle_list[i][j+1].y << std::endl; 
        }
    }
  ...
  for(int i=0;i<3;i++){
    ...
    std::cout <<"Dubins  start_point.x= " << three_seg[i].start_point.x << "start_point.y= " << three_seg[i].start_point.y<< "end_point.y= " << three_seg[i].end_point.x << "end_point.y= " << three_seg[i].end_point.y << std::endl; 
...
void UT_line_arc_collision_prof(img_map_def *map_param){
  ...
  if(intersect){
    std::cout <<"UT_line_arc_collision line-Arc INTERSECTION  " << std::endl;
  ...
  {
    std::cout <<"UT_line_arc_collision line-Arc NO INTERSECTION  " << std::endl;
  }
...
void UT_Bounding_Box_line_check(std::vector<Polygon> obstacle_list,img_map_def *map_param){
  ...
  if(Intersection)
      std::cout <<"UT_Bounding_Box_line_check line-line INTERSECTION  " << std::endl;
  else
      std::cout <<"UT_Bounding_Box_line_check line-line NO INTERSECTION  " << std::endl;
...
void UT_Bounding_Box_arc_check(std::vector<Polygon> obstacle_list,img_map_def *map_param){
  ...
  if(Intersection)
      std::cout <<"UT_Bounding_Box_line_check line-arc INTERSECTION  " << std::endl;
  else
      std::cout <<"UT_Bounding_Box_line_check line-arc NO INTERSECTION  " << std::endl;
...
void UT_Global_line_collision_check(std::vector<Polygon> obstacle_list,img_map_def *map_param){
  ...
  if(Intersection)
      std::cout <<"UT_Global_line_collision_check COLLISION  " << std::endl;
  else
      std::cout <<"UT_Global_line_collision_check NO COLLISION  " << std::endl;
...
void UT_Global_arc_collision_check(std::vector<Polygon> obstacle_list,img_map_def *map_param){
  ...
  for (size_t i = 0; i<obstacle_list.size(); i++){//no of boxes 
        for (size_t j = 0; j<obstacle_list[i].size()-1; j++){ //4corners of each box
            construct_line_structure(line_data,obstacle_list[i][j],obstacle_list[i][j+1]);
            draw_line(line_data, *map_param, cv::Scalar(0,255,0)); 
            //std::cout <<"Ambikeya:Polygon start_point.x= " << obstacle_list[i][j].x << "start_point.y= " << obstacle_list[i][j].y<< "end_point.y= " << obstacle_list[i][j+1].x << "end_point.y= " << obstacle_list[i][j+1].y << std::endl; 
        }
  }

  Intersection=Global_Arc_check(obstacle_list,arc_seg);
  if(Intersection)
      std::cout <<"UT_Global_arc_collision_check COLLISION  " << std::endl;
  else
      std::cout <<"UT_Global_arc_collision_check NO COLLISION  " << std::endl;
...
void UT_permute(){
  ...
  std::vector<int> comb;
  std::vector<std::vector<int>> v_comb;
